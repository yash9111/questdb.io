...
tags:
  [time-series, time-series data, tsdb, time-series database, SQL]
...
<Screenshot
  ...
  width={892}
  height={432}
  src="/img/blog/2023-01-17/btc-eth-chart.png"
  title="Chart with price of Bitcoin and Ethereum"
/>

Financial markets are the birthplace of time-series data. The price of securities
change quickly, and every increment is known as tick data. Those increments can be
in milliseconds, microseconds or even nanoseconds. 

Financial exchanges expose the price of stocks, but there is also a broader spectrum
of securities, such as commodities and bonds. Since 2009, a new asset class has emerged
with cryptocurrencies such as Bitcoin and Ethereum.

### Sensor data and industrial analytics

Time-series data is a critical component in various industries, including energy,
space exploration, telco, and manufacturing. 
...
In space exploration, sensors and monitoring devices are installed throughout the rocket,
including the engine. This data includes critical parameters such as pressure, temperature,
fuel consumption, and thrust, which are recorded over time.
The data is used to monitor the rocket's performance, optimize trajectory and fuel consumption,
...
the user journey. In e-commerce, companies track and react to consumer behaviors dynamically,
in order to automate marketing campaign triggers. To do so, they record online order tracking,
keyword searches, and log user interaction with online advertisement. 

These analytics are often displayed in real-time on a dashboard such as Grafana. 

Other industries of choice for analytics include cyber security (network flow data),
DevOps (servers), SaaS apps and Web3.
...
The integration of GPS sensors into a fleet of delivery trucks enables 
real-time tracking of their location, speed, and direction. The resulting time-series data
is typically transmitted to a central server for analysis. By analyzing this data, logistics
and mobility operators can optimize delivery schedules, reduce fuel consumption, and monitor fleets behavior.

Additionally to timestamped data, geospatial data is also in use to track the location of 
objects on a map. QuestDB features the [geohash data type](/docs/concept/geohashes/)
natively.

### Machine learning forecasting 

Machine learning forecasting models use time-series data to predict future values of a variable
by learning from past patterns and trends. These models take into account seasonality, 
trends, and fluctuations in the data to make accurate forecasts. 
Prophet is an example of a forecasting model developed by Facebook.
...
- **Streaming ingestion protocols**
  ...
  [InfluxDB line protocol](/docs/reference/api/ilp/overview/) is very
