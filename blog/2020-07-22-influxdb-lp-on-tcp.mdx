...
keywords:
  ...
  - influxdb
  - ilp
tags: [tutorial, questdb, influxdb, ilp, telegraf]
...
to ingest data by listening for line protocol packets over TCP. This tutorial
will walk you through the basics of setting up and using ILP. For an added
bonus, we'll see how to migrate from using InfluxDB to using QuestDB with a
...
`tags`. The second space separates the values from the timestamp for those tags
and values.
...
If you're relying on QuestDB to add `timestamps`, then be aware that QuestDB
will give the entire batch sequential timestamps based on when they are
read/written to the database.
...
That's it. That's the migration. All data that was previously written to
