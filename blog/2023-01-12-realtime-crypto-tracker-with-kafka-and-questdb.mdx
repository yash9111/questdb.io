...
With the connector, we send the payload directly:
...
For the Kafka Connect configuration, we can specify the new fields:
...
- `value.converter.schemas.enable`: since schema is not included, we set the
  converter to false
